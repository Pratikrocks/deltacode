...
class TestDeltacode(FileBasedTesting):
    ...
    def test_Delta_one_None(self):
        ...
        print(expected_first,"**", first_None.to_dict(deltacode))
    ...
    # TODO : I will modify it by creating a separate jsons and remove the File attributes
    ...
    def test_Delta_to_dict_empty(self):
        ...
        assert delta.to_dict(deltacode) == OrderedDict([
            ('status', ''),
            ('factors', []),
            ('score', 0),
            ('new', None),
    ...
    def test_score_new_multiple_keys_same_lic(self):
        ...
        assert [d.to_dict(deltas_object).get('new').get('licenses') for d in deltas_object if d.new_file.path == 'path.txt'].pop() == [
            OrderedDict([
                ('key', 'mit'),
                ('score', 75.0),
                ('short_name', None),
    ...
    def test_Delta_to_dict_Copyright_unusual_characters(self):
        ...
        assert [d.score for d in deltas_object if d.new_file.path == 'a1.py'] == [55]
    ...
    def test_Stat_calculation_and_ordering(self):
        ...
        stats_object = deltacode_object.stats.to_dict(deltacode_object)
